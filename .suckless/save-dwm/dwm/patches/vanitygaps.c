...
void toggle_always_smartgaps_monocle(const Arg *arg) {
...
static void bstack(Monitor *m) {
  ...
  // Уменьшаем высоту на bottGaps
  sh = mh = m->wh - bottGaps - 2 * oh;
...
static void bstackhoriz(Monitor *m) {
  ...
  if (m->nmaster && n > m->nmaster) {
    ...
    sy = my + mh + ih;
    sh =
        m->wh - bottGaps - mh - 2 * oh - ih * (n - m->nmaster); // учли bottGaps
  }
...
void centeredmaster(Monitor *m) {
  ...
  mh = m->wh - bottGaps - 2 * oh -
       ih * ((!m->nmaster ? n : MIN(n, m->nmaster)) - 1);
  mw = m->ww - 2 * ov;
  lh = m->wh - bottGaps - 2 * oh - ih * (((n - m->nmaster) / 2) - 1);
  rh = m->wh - bottGaps - 2 * oh -
...
void deck(Monitor *m) {
  ...
  sx = mx = m->wx + ov;
  sy = my = m->wy + oh;
  sh = mh = m->wh - 2 * oh - bottGaps -
            ih * (MIN(n, m->nmaster) - 1); /* Учет bottGaps */
  sw = mw = m->ww - 2 * ov;
...
void monocle(Monitor *m) {
  unsigned int n;
  int oh, ov, ih, iv; // Внешние и внутренние отступы
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    resize(c,
           ...
           m->wh - bottGaps - 2 * oh -
               2 * c->bw, // высота с учетом горизонтальных отступов и bottGaps
           0);
  }
}

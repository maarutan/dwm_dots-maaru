...
void toggle_bottGaps(const Arg *arg) {
    ...
    if (bottGaps == 0) {
        // Если отступы 0, восстанавливаем сохранённое значение (например, 160 или
        // то, что было сохранено)
        bottGaps = bottgaps;
...
void save_showtitle_state(void) {
    ...
    if (file != NULL) {
        ...
        fprintf(
            stderr,
            "Ошибка: не удалось открыть файл для записи состояния showtitle.\n");
    }
}
...
void load_showtitle_state(void) {
    ...
    if (file != NULL) {
        if (fscanf(file, "%d", &showtitle) != 1) {
            fprintf(stderr,
                    "Ошибка: не удалось считать состояние showtitle из файла.\n");
        }
        fclose(file);
    } else {
        fprintf(
            stderr,
            "Ошибка: не удалось открыть файл для чтения состояния showtitle.\n");
    }
}
...
void saveSmartgapsState(int state) {
    ...
    if (file) {
        ...
        fprintf(
            stderr,
            "Ошибка: не удалось открыть файл для записи состояния smartgaps.\n");
    }
}
...
int loadSmartgapsState() {
    ...
    if (file != NULL) {
        ...
        fprintf(stderr,
                "Ошибка: не удалось открыть файл для чтения состояния. "
